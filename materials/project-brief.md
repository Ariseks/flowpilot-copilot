...
**当前实现版本：0.2.0。** 本轮重点补齐了可配置证据门槛、失败上下文 Trace 和任务检索重跑，同时保持单机 MVP 与“只生成建议、不执行外部写操作”的边界。
...
- `materials/` 仅保留本项目说明 `project-brief.md`；已删除的面试记录、业务原始材料和不适合公开的文件不纳入代码仓库，也不作为运行时知识库输入。
...
本轮改动以本 brief 的功能边界为准，未引入新的外部数据源或副作用操作：

1. **统一证据边界**：新增 `EVIDENCE_THRESHOLD` 配置（默认 `0.1`），Agent、普通 Copilot 和 LangChain 适配链路统一以 Top-1 TF-IDF 相关度判断知识证据；BM25/RRF 仅负责排序，低相关片段不会传给模型。
2. **可审计 Trace**：Agent 任务记录原始候选、门槛和判定状态、请求意图/Top-K/策略、Prompt 与生成器版本、实际 Prompt（有长度上限）、反馈上下文快照和重跑来源。
3. **可配置任务重跑**：`POST /api/agent/tasks/{task_id}/replay` 读取原任务输入，允许覆盖 `tfidf`、`bm25`、`rrf` 和 Top-K，并通过 `trace.replay_of` 关联新旧任务；历史 `auto` 意图也可正常重跑。
4. **评估口径对齐**：Golden Dataset 的 baseline/hybrid 命中、来源召回和 MRR 均使用同一证据门槛，避免原始候选与最终可用证据口径不一致。
5. **可见状态一致**：当云模型未实际产出内容（证据拒绝或调用失败）时，接口和 Trace 标记实际展示结果为 Demo/本地降级；这不代表云模型配置不存在。

以上能力仍属于本地 JSON MVP：没有登录、RBAC、租户隔离、数据脱敏审批、长期保留策略、Embedding 向量库、Reranker、LangGraph 检查点或外部系统写入。它们保留在生产化迭代清单中，需在出现明确业务需求后单独设计和验证。
